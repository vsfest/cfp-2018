    ...
    return (
      <Form onSubmit={this.onSubmit.bind(this)} inProgress={inProgress}>
        ...
        {submission.example ? (
          ...
          <Para>
            This year we're doing things a little differently, you should have a{' '}
            <Link external href={blogPostUrl}>
              read about why
            </Link>. <strong>Submissions are open until December 1st.</strong>
          </Para>
        )}
        <hr />
        <TextField name="title" label="Title" form={submission} />
        <Field>
          <Field.Label>Talk Description</Field.Label>
          <Field.Note>
            <strong>NOTE</strong> please replace anything
            personally-identifiable in your talk submission with a string like
            COMPANY_A, PROJECT_B, PERSON_C, or OTHER_D.
          </Field.Note>
        ...
        <BooleanCheckBox
          name="submitToBoth"
          label={`Would you like this talk to be considered for ${store
            .conference.id === 'js'
            ? 'CSSConf'
            : 'JSConf'} as well?`}
        ...
        <RadioButton
          ...
          options={speakingExperienceOptions}
          form={submission}
        />
        <RadioButton
          name="flights"
          label="Can your company pay for flights"
          options={['Yes', 'No']}
          form={submission}
        />
        <TextField
        ...
        <TextField
          type="text"
          name="twitter"
          label="Twitter Handle or Website"
          form={submission}
          required={false}
        />
        <TextField
        ...
        />
        <BooleanCheckBox
          name="readTheCodeOfConduct"
          text="Yes, and I agree to be bound by it."
          required
          form={submission}
        >
          Have you read our{' '}
          <Link external href={`${store.conference.url}/code-of-conduct`}>
            Code of Conduct
          </Link>
        </BooleanCheckBox>
        {submission.example ? null : (
          <Form.Buttons>
            <Para>
              Once you're happy with your submission, send it to us. You will
              receive a link to access your proposals afterwards.
            </Para>
            <Button type="submit">
              {submission.inProgress
                ? 'Saving...'
                : submission.sekret ? 'Save' : 'Submit'}
            </Button>
          </Form.Buttons>
        )}
      </Form>
